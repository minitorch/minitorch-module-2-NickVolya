...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return t1.f.mul_zip(t1.f.mul_zip(sigmoid, 1 + t1.f.neg_map(sigmoid)), grad_output)
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        inv_per = np.argsort([int(order[i]) for i in range(order.size)])
        return grad_output._new(grad_output._tensor.permute(*inv_per)), 0.0
